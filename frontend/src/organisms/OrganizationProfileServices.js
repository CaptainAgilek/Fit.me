...
export function OrganizationProfileServices(
  ...
  if (
    (!organizationData) ||
    (servicesState.loading && !servicesState.servicesState.data)
  ) {
    return <Loading />;
  }

  // console.log('data');
  // console.log(servicesState.servicesState.data);
  // console.log(organizationData.organization.places[0].place_id);
  // console.log(organization);

  let tmpList = [];
  let servicesList = [];
  if (servicesState.servicesState.data) {
    tmpList = servicesState.servicesState.data.servicesForPlace;
  ...
  return (
    <Container>
      {servicesState.servicesState.loading && <Loading />}
      {servicesState.servicesState.error && <ErrorBanner message={servicesState.servicesState.error.message} />}

      {/*List of services*/}
      {servicesState.servicesState && servicesState.servicesState.data && organizationData &&
              ...
              >
                <button onClick={() =>
                  deleteServiceRequest({
                    variables: { place_id: service.place_id, service_id: service.service_id },
                  })
                }>
                  SMAZAT
        ...
        <button onClick={handleClick}>
          <div className="category-box">
            <div className="category-box-img">
              <img className="img-fluid align-content-center" src={'/images/plus.png'} alt="Přidat službu" />
            </div>
          </div>
        </button>
      </div>
